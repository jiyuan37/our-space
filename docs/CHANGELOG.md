...
- prompt v3禁止新增自拍中没有的配饰，图1唯一决定身份，图0仅供风格；参考副本移除发夹，原型不变。
- 本人身份拒绝入口与持久rejectionReason，服务端禁止确认/离线恢复撤销拒绝；当前真实候选已标记身份不符，正式头像未变。
- 双语/键盘/持久化/权限回归，0真实请求；记录见AVATAR_IDENTITY_GUARD_2026-09-06.md。
...
- 删除 FLUX 额外风格参考图外发，更新中英文说明及 provider 输入白名单测试；不修改用户环境配置或正式头像。
- 本轮未保留内存候选做人工确认，不能判定相似度/画风达标，也不能宣告 AVATAR-01 完成。具体证据与限制见 AVATAR_SMOKE_2026-09-05.md。
...
- 保持手机 60×62.5px、原地理锚点和背景/布局/移动状态机。小林长发专注看书，小雨短发微笑喝奶茶；头部可见面积约 84.4% / 82.9%。
- 新增从实际旧/新代码生成原尺寸、4× 最近邻并排对照的脚本；地图 before/after 同视角同缩放，产物在仓库外。
- 23/23 状态机与 31 组浏览器检查通过，保留原有 30 组语义；format 与 diff 检查通过。生产 `/home`、数据库、依赖、真实头像生成与定位均未动，局部人物美术等待用户确认。
