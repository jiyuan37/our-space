...
详细需求状态、依赖、隐私条件、建议验收证据和未决问题集中记录于 [`AVATAR_AND_MAP_SPEC.md`](./AVATAR_AND_MAP_SPEC.md)。原型时期进一步批准具体规格、研究与隔离原型；2026-09-05 最新授权见第 30 节，AVATAR-01 已批准正式实现，ANIMATION-01、MAP-01 本轮不实施，完整 Phase 4 未开始。已完成 Phase 1–3 的历史验收继续有效。
...
- `AVATAR-01`：原创像素大头持久身份，采用第 30 节已批准比例与轮廓；首次进入突出引导，自拍/上传 → AI 候选 → 选择/调整/重试 → 本人明确确认。当前非阻断入口及头像专用外部处理规则见第 30 节。
...
# 30. AVATAR-01 正式实施授权（2026-09-05）

本节记录用户最新明确指令，覆盖原型阶段的未批准状态，不改写 Phase 1–3 历史验收。

- 用户已确认 `b6fe15a8c11270d3c1568f7f40af08484ce71fd3` 的原创温暖像素视觉。沿用圆润大头、阶梯脸颊/下巴、清楚表情、极少肩部，不再全身或普通圆形照片。基础身份没有永久状态道具；后续同角色状态最多一个道具，ANIMATION-01 另行实现。
- 当前唯一工作包 AVATAR-01：真实用户上传自己的照片，经明确同意生成候选，选择/重试/取消，明确确认后持久绑定本人 Resident，在正式 Home 与另一位 ACTIVE Resident 读取中使用。没有确认不能自动替换；失败/取消保留旧头像。
- 创建入口显著且非阻断；沿用注册、登录、Space 与 Invitation，不增加完成度/任务/奖励或示例人物分配。
- 用户暂不批准 OpenAI Image API 付费接入，要求可替换 AvatarGenerationProvider，先 Cloudflare Workers AI 免费路线 SDXL-Lightning img2img；人物一致性不足再测试 FLUX.2 klein。最多 2 张另行明确授权测试照片。服务账户配置不等于照片授权。
- 只外发去 EXIF/缩小后的本人照片、固定头像提示和必要的已批准原创风格参考。不得发送姓名、位置、Presence、伴侣、生活记录或其他 Space 内容。原照不落盘；候选最多 24 小时、取消删除；最终确认后私密持久保存，替换删除旧图，应用图片存储排除备份。Cloudflare 服务侧具体保留时限/地域不作未经证实的保证。
- 允许必要的正式应用改动、最小新增 migration、可替换私密存储与 provider、服务端权限/限流/幂等、双语与相关验证。不开地图生产接入、定位采集、模拟轨迹或新的核心实体。
- 真实 provider 或授权样本未解决时，必须报告闭环未完成。受控 fixture 仅自动测试，不能作为 AI 成果；候选透明处理、资源版本不等于可拆层/骨骼动画。

实现、当前限制与可复现证据见 AVATAR_OPERATIONS.md、CURRENT_STATE.md 与 IMPLEMENTATION_PLAN.md。
