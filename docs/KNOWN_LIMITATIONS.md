...
- `/home`底图已迁移至OpenFreeMap vector tiles，public Overpass不再是首屏/Home依赖；既有Overpass能力仅为可选enrichment。当前执行环境的网络代理拒绝连接`tiles.openfreemap.org`（403），真实tile加载与截图仍须在可访问网络完成，不能以fixture替代。
