...
## DEC-048 — 持久卡通身份是已确认的必做产品能力

- 日期：2026-09-04
- 状态：已接受；产品要求已确认，详细设计待收口，尚未实施
- 决策：Our Space 必须让 Resident 建立本人明确确认且后续持续复用的卡通视觉身份。目标流程包含自拍或上传照片、AI 生成卡通候选、选择/调整或重新生成、明确确认满意结果，以及后续持续使用同一个角色。注册后或首次进入 Space 时必须显著引导，不能仅藏在低频 Settings。已有 `avatarUrl` 或一张静态头像不等同于完整能力。是否阻断 Home、是否必须使用真人照片，以及拒绝、失败、不满意或撤回路径仍待设计；本决定不授权实施。
- 理由：用户明确把持续卡通身份确认为核心产品要求；稳定身份是后续状态动画成立的前提，而突出引导与最终确认保证它不是隐藏装饰。
- 考虑过的替代方案：
  - 只使用现有 `avatarUrl` — 拒绝，因为缺少生成、选择/调整、确认与持续身份流程。
  - 每次状态更新重新生成不同人物 — 拒绝，因为破坏 Resident 的持续视觉身份。
  - 将入口只放进 Settings — 拒绝，因为不满足显著创建引导。

## DEC-049 — 状态动画必须保持角色身份并只表达用户主动状态

- 日期：2026-09-04
- 状态：已接受；产品要求已确认，详细设计待收口，尚未实施
- 决策：后续状态动画必须使用 Resident 已确认的同一角色，通过动作或表情表达用户主动留下的当前状态。它不代表实时位置、online/offline、后台行为监控或情绪推断；Presence 清除或不再满足 viewer-local-day freshness 后，不得继续表现旧状态为当前活动。必须提供 reduced-motion 与等价静态展示。具体动作库、状态映射、动画技术和逐次确认方式待设计；本决定不授权实施。
- 理由：动画需要增加共同空间的 Presence，而不是制造监控或身份漂移；Accessibility 与已接受的 Presence freshness 必须从设计起保持一致。
- 考虑过的替代方案：
  - 用单张静态头像宣告动画完成 — 拒绝，因为没有交付动态状态表达。
  - 从设备或后台行为自动推断状态 — 拒绝，因为违反非监控边界。
  - 用奖励、等级、装扮经济或更新 streak 驱动动画 — 拒绝，因为会制造 engagement 与关系压力。

## DEC-050 — 地图与标记／状态点是核心空间表达

- 日期：2026-09-04
- 状态：已接受；产品要求已确认，详细设计待收口，尚未实施
- 决策：地图及其标记／状态点必须作为 Our Space 的核心空间表达交付，不能被降级为可无限延期的边缘附加页。地图是地理、抽象共同生活空间或混合表达，及其与 Home/navigation、Resident、Presence、LifePoint、SharedMoment 的具体关系仍待设计。地图不自动授权持续定位；Presence 当前状态与 LifePoint 生活记录保持概念区分，所有状态点不得被预先等同于 LifePoint，LifePoint 继续不强制 location。本决定不授权实施。
- 理由：用户明确要求地图成为产品空间感的核心组成，同时现有六实体语义与隐私原则不足以安全推断 marker 和位置行为。
- 考虑过的替代方案：
  - 把地图列为 optional/post-MVP enhancement — 拒绝，因为会降级已确认必做范围。
  - 默认采用真实地理地图和后台定位 — 拒绝，因为用户没有批准位置收集或技术路线。
  - 把普通 Presence 列表或所有 LifePoint 称为状态点 — 拒绝，因为没有完成地图及 marker 语义。

## DEC-051 — Avatar 产品目标不自动授权外部 AI 处理

- 日期：2026-09-04
- 状态：已接受；外部处理例外待决定，尚未实施
- 决策：AVATAR-01 对“自拍/上传照片 → AI 生成候选”的产品目标确认，不等于批准向外部服务发送自拍、原图、Presence 或共同生活内容。在任何外部处理前，必须明确并获得批准：最小输入范围、是否确需外部 provider、provider 与处理地域、知情同意、传输/训练/保留/subprocessor 行为、原图与生成资源的访问/保存/删除、失败/不满意/拒绝/撤回路径，以及日志数据最小化。即使未来批准 avatar-specific 例外，也不得把 Presence、LifePoint、Response、SharedMoment、Space 内容或关系数据作为隐含输入。
- 理由：旧规格的外部 AI 隐私限制不能被用来遗漏头像需求，也不能因头像需求而被整体删除；最小且显式批准的用途边界可以同时保留产品目标与用户控制。
- 考虑过的替代方案：
  - 立即选择并调用外部 avatar provider — 拒绝，因为尚无 provider、同意、保留与删除批准。
  - 删除全部 AI/隐私限制 — 拒绝，因为会无必要扩大敏感数据暴露。
  - 将 Space 内容用于提升生成效果 — 拒绝，因为超出头像最小用途且违反私密边界。

## DEC-052 — 真实世界像素地图成为未来 Home 主体

- 日期：2026-09-04
- 状态：已接受；产品方向明确，具体视觉与生产方案待用户确认
- 决策：取代 DEC-050 中地图类型/入口未定的部分：地图对应真实道路、水系、公园和地点，作为 Home 全屏主体；不新增次级 Map 页面。采用像素轮廓与柔和线条结合的原创 Q 版人物，不复制第三方角色/品牌/代码。后续替代 DEC-044 的文字窄幅布局，保留其 Quiet Home、Silence、隐私与无压力哲学；既有 Phase 3 验收不变。
- 理由：用户已明确选择，不应重复要求其选择地图类型或旧 C/B 方向。具体规格与编号集中于 AVATAR_AND_MAP_SPEC.md。

## DEC-053 — 可信位置变化与自愿 Presence 动画分别建模

- 日期：2026-09-04
- 状态：已接受；语义已收口，生产未实施
- 决策：补充 DEC-049；Presence 的非监控/当天规则继续有效，GPS 不更新 Presence.updatedAt。位置回放由本人授权且已经记录的有效变化触发，非欢迎动画，无变化不走，无基线不虚构起点，无数据不暗示未移动；端点示意、有效采样段、缺口和长距必须诚实区分。八帧步态、3 秒回放及过滤参数仅为原型打样。
- 理由：查看者打开 App 不能创造另一设备未采集的路线，且走动表达不证明交通方式。完成/跳过游标是查看者自身状态，不作为对方可见的位置已读。

## DEC-054 — 位置分享独立授权及后台能力边界

- 日期：2026-09-04
- 状态：本人知情自主控制要求已接受；精度/频率/保留/provider/原生方案提议中
- 决策：被定位本人开启、暂停和撤销，OWNER 不能代开；照片与位置不捆绑授权。epoch 隔离暂停前后，服务端逐次权限与缓存访问复查，位置样本不创建 LifePoint。Web/PWA 无法保证隐藏、锁屏、关闭后的持续采样，完整后台目标需要原生定位工作包和 native 排除项的范围修订。没有授权创建原生工程。
- 理由：基于 W3C、Chrome、Apple、Android 官方资料，不能声称 watchPosition/Service Worker 足以持续后台记录；也不能把产品目标缩成双方一直开网页。具体证据与建议在 AVATAR_AND_MAP_SPEC.md。

## DEC-055 — 本轮只实施隔离交互原型并验证

- 日期：2026-09-04
- 状态：已接受；本轮原型实施已批准，生产未批准
- 决策：允许 prototypes/map-home/ 下独立 HTML/CSS/JavaScript、公开合规地理数据、原创示例 SVG、独立测试/截图和相关文档。静态本地服务器与生产 Next.js 隔离，不更改 src/、根依赖、配置、数据库或生产测试。验证后提交 design: specify and prototype pixel map home 并正常 push origin/main；完成后停止。
- 理由：用户要求可打开体验的具体结果，明确禁止把原型宣称为头像生成、真实共享、后台轨迹、生产 Home 或 Phase 4 完成。

## DEC-056 — 真实地图采用星露谷式温暖像素视觉

- 日期：2026-09-04
- 状态：用户补充方向已接受，原型视觉待确认
- 决策：真实地理关系不变，使用原创像素草地、水纹、树冠、屋顶与 Q 版角色表达用户指定的《星露谷物语》式温暖乡野感。装饰树木不是实测 POI，不使用游戏原资产，不引入任务、奖励、打卡或关系计分。
- 理由：用户明确要求真实地图的风格转译；视觉层可以风格化，现实道路与地点不能挪动。

## DEC-057 — 已批准大头像素视觉与 AVATAR-01 正式范围

- 日期：2026-09-05
- 状态：用户明确批准实施；真实生成验收尚未完成
- 决策：用户确认 `b6fe15a8c11270d3c1568f7f40af08484ce71fd3`。沿用原创温暖大头像素角色，清楚表情、极少肩部，基础身份不永久绑定状态道具。非阻断 `/home` 显著创建/更换入口，`/avatar` 完整选择照片、知情同意、生成、候选选择、确认、重试与取消。当前只做 AVATAR-01，地图/定位/ANIMATION-01 不随本轮实施。
- 理由：用户明确要求推进可使用产品，禁止再次风格冻结或仅 provider interface 宣称完成。覆盖 DEC-055 原型轮次的范围限制，不改变历史验收。

## DEC-058 — 可替换 Cloudflare 免费头像处理

- 日期：2026-09-05
- 状态：用户指定路线已接受；真实账户、授权样本、质量 smoke 尚未验证
- 决策：不接 OpenAI 付费方案。Service 依赖 `AvatarGenerationProvider`；优先 `@cf/bytedance/stable-diffusion-xl-lightning` 的照片 img2img，FLUX.2 klein 4B 仅显式备选。一次任务一个 1024px 候选、无自动重试或后备切换；同一请求 UUID 共享数据库防重复，滚动 24 小时 3 次/人和 20 次/全站。仅允许运营者明确配置的 Workers Free，不能把配置值称为实际账单计划已验证。
- 外发边界：解码、去 EXIF、缩至最长 1024px 的本人照片与固定提示；FLUX 输入再缩至 480px，附原创建模风格参考。没有 User/Resident 姓名、Presence、位置、伴侣、Space 内容。不使用 AI Gateway/R2/KV 存储。
- 数据：应用原照仅请求内存，不落盘；候选最多 24 小时且本人可见；明确确认后保存最终图、替换清理旧图。Cloudflare 官方声明未经同意不训练/改进，未核实逐项保留/删除时限及固定地域，因此不承诺零保留或地域。官方能力/价格/数据链接与查询日期见 AVATAR_OPERATIONS.md。
- 理由：用户暂不批准付费，要求先用最多两张明确授权照片比较像本人和像素风一致性。当前无实际照片调用，不把固定素材或接口测试视为模型质量验收。

## DEC-059 — Resident 确认指针、私密资源与生命周期

- 日期：2026-09-05
- 状态：实现并以受控 provider 验证；部署与真实服务条件另列
- 决策：复用 Resident 与 MediaAsset；新增支撑性 AvatarGeneration（非第七核心实体）记录请求、模型、同意策略、基准身份版本、派发/到期及候选指针。Resident 仅在本人确认事务中更新 avatarMediaAssetId/递增 avatarVersion；当前确认记录持续保留，旧无资源记录及失败/取消元数据 7 天后清理。`avatar-cloudflare-v1` 对应 `pixel-big-head-b6fe15a-v1`，后续提示/处理版本变更须使用新版本关联。
- 授权：无头像使用姓名默认表示，不读取任意 legacy avatarUrl；候选只本人 ACTIVE membership 可读，最终图仅当前同 Space ACTIVE membership 可读。每次资源请求验证 session 与 Space，private/no-store，无公开静态目录、带令牌公开地址或服务密钥下发。
- 生命周期：取消/替换在数据库撤销旧资源可读性后删除文件；过期立即拒绝读取/确认。常驻 Node 进程每 60 秒清理，停机后重启补扫；文件删除失败由下一次孤儿扫描补偿（文件创建超过 1 小时），不可把停机状态称为硬实时物理删除。私密持久目录必须排除备份；当前仓库没有部署备份配置，需部署者实际落实。
- 并发：短 PostgreSQL advisory transaction lock 保护派发、全站限额、确认与取消；不在数据库事务内等待 AI；取消墓碑阻止先取消后到达的同 id 请求，迟到生成结果删除。候选确认检查生成时 avatarVersion，失败/未确认不覆盖旧身份。
- 理由：持久视觉身份属于真实 Resident，未确认候选和测试示例不得被伴侣误认；保留实际资源与版本关联即可，不假定可拆层或已支持动画。

## DEC-060 — 有限真实调用与更小外发输入范围

- 日期：2026-09-05
- 状态：用户最多 2 次真实 smoke 授权已执行；AVATAR-01 人工验收仍未完成
- 决策：本轮只发送明确授权样本 1（样本 2 不存在），两次总上限包含失败请求。SDXL-Lightning 返回 400 / 3030 后，以剩余一次测试 FLUX.2 klein，返回 200 并通过当前规范化；不重试、不追加第三次、不因成功自动替换正式身份。
...
## DEC-063 — 自适应纯色背景后处理与图0风格/图1身份（2026-09-06）

用户明确批准本轮离线处理已保存真实source并修正双参考输入；禁止新真实请求。保留高质量source，显示图去背景在全分辨率完成，256px PNG不再强制64px/64色。仅连通背景去除与局部边缘混色处理，不能可靠分离时失败，不靠抹除主体凑成功。当前真实图恢复READY不等于风格已通过，不自动确认。源图/候选仍本人私密、24h/取消删除。

用户明确指定input_image_0为已批准原创原型的无道具风格参考，input_image_1为自拍身份。模板版本flux-style0-identity1-v2，双边<512、无guidance、无新增Space信息；取代DEC-060期间的自拍单图白名单，双语处理说明同步。仅配置和测试完成，真实有效性未验证，下一次必须另获单次授权。详情见AVATAR_OFFLINE_NORMALIZATION_2026-09-06.md。

## DEC-064 — 身份事实优先与人工拒绝（2026-09-06）
...
## DEC-067 — 有界cell与真实完整性门槛（2026-09-07）
...
## DEC-069 — Public Overpass仅作受控诊断来源，不作为MAP-01A生产SLA
